...
const DEBUG_GROUPS = [
  ...
  },
  {
    ...
    rows: [
      ...
      { id: 'tx-haptic', k: 'debug.tx.haptic', v: d => count(sub(d, 'tx').hapticAudio) },
  ...
  },
  {
    ...
    rows: [
      ...
      { id: 'au-hap-sent', k: 'debug.audio.haptic-sent',
        v: d => count(sub(d, 'audio').hapticSent) },
      { id: 'au-hap-reduced', k: 'debug.audio.haptic-reduced',
        v: d => count(sub(d, 'audio').hapticReducedToRumble) },
      { id: 'au-hap-silence', k: 'debug.audio.haptic-silence',
        v: d => count(sub(d, 'audio').hapticSilenceSuppressed) },
      { id: 'au-hap-encfail', k: 'debug.audio.haptic-encode-fail',
        v: d => countCls(sub(d, 'audio').hapticEncodeFailed, 'debug-err') },
      { id: 'au-hap-lock', k: 'debug.audio.haptic-contended',
        v: d => countCls(sub(d, 'audio').hapticLockContended, 'debug-warn') },
    ],
  },
  {
