...
public:
    ...
    void updateClientAddr(uint32_t token,
    ...
    void sendControllerAck(const Connection& conn,
                           uint16_t requestType,
                           uint8_t ctrlIdx, uint8_t result) override;
    void sendServerStatus(const Connection& conn,
                          bool vigemAvailable,
...
private:
    ...
    void sendEncryptedPacket(const Connection& conn,
