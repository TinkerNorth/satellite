...
extern bool encryptPacket(const uint8_t key[32], uint32_t counter,
                          uint32_t token,
                          const uint8_t* plaintext, size_t ptLen,
                          uint8_t* ciphertext, unsigned long long* ctLen);

void ClientAdapter::setSocket(SOCKET sock) {
    sock_ = sock;
}

void ClientAdapter::updateClientAddr(uint32_t token,
...
void ClientAdapter::sendEncryptedPacket(const Connection& conn,
    ...
    if (!encryptPacket(conn.sharedKey, 0, conn.token, inner, innerLen, ct, &ctLen))
        return;
    ...
    sendto(sock_, reinterpret_cast<const char*>(pkt),
           (int)(HEADER_SIZE + ctLen), 0,
...
void ClientAdapter::sendControllerAck(const Connection& conn,
                                       uint16_t requestType,
                                       uint8_t ctrlIdx, uint8_t result) {
    uint8_t inner[8];
...
void ClientAdapter::sendServerStatus(const Connection& conn,
                                      bool vigemAvailable,
...
void ClientAdapter::broadcastServerStatus(
        ...
        bool vigemAvailable, uint8_t totalActiveControllers) {
