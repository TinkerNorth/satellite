...
// own copy/remediation table off (id, errorCode); each platform owns its
...
// deliver (macOS has no gamepad bus → Pad and Mouse both no-op).
//   Pad   — needs a backend carrying a DS4 touchpad surface (ViGEm / uinput).
//   Mouse — needs host-global pointer injection (SendInput / uinput EV_REL).
...
// probeBackend(). Pad+mouse are tied to having any gamepad backend at all:
